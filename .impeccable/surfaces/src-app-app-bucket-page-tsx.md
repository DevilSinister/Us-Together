...
- **FIRST VIEWPORT:** The list title and real Add an idea action establish the job; item detail prioritizes the idea and Plan this/Mark complete actions.
- **FORM:** Preserve the user's Options-based layout and in-place idea editor. Ideas remain spacious rows; list selection, filtering, creation and management share one dialog with explicit Back navigation. Applied filters have removable chips; first-run has a direct create-list action. Mobile keeps bottom navigation and one concise step row; reorder arrows and drag handles remain desktop-only. No replacement visual world is introduced.
