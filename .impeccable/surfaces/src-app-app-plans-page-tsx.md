...
- **Changes and recovery:** Dedicated creation/edit routes keep timezone and clock-repeat choices explicit. Cancellation keeps details and stops pending reminders; cancelled plans can be restored. Edit plan and Delete plan share the foot row (2026-09-25, ADR-038); Delete opens the shared confirmation dialog, which names any attachments that must go first and explains that saved memories remain. Attachment removal also confirms. Pending and failed actions receive visible feedback.
...
The new-plan page opens with a blush "Plan something from your bucket list" panel: a native select of every not-yet-completed idea, grouped by list, submitted as a plain GET form. With an idea chosen the page becomes "Give someday a date." and the ordinary plan form is filled from the idea: title, story, place, target date at noon, estimated cost and currency, and a kind guessed from the category. It offers "Use this idea instead" and "Start from scratch". The map-link field is hidden for a sourced plan because the bucket RPC stores none. Plan kinds read as labels ("Date", "Trip") rather than stored values.
