...
export default async function NewMemoryPage({ searchParams }: { searchParams: Promise<{ plan?: string; bucket?: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl reveal-on-load"><Link href="/memories" className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-primary hover:underline"><ArrowLeft className="size-4" />Back to memories</Link><header className="mt-5"><p className="text-sm font-semibold text-primary">Keep what mattered</p><h1 className="mt-2 font-display text-5xl tracking-[-0.03em] sm:text-6xl">Bring the moment home.</h1><p className="mt-4 max-w-[65ch] text-lg leading-8 text-muted-foreground">Write the detail first. Private media metadata and storage boundaries are ready for the attachment step that follows.</p></header><section className="mt-9 rounded-[1rem] bg-card p-6 sm:p-8"><MemoryForm sourcePlan={sourcePlan} /></section></div>;
}
