...
import { planTypeLabel, planTypes, zonedLocalToUtc } from "@/lib/dream/schema";
import type { BucketItem } from "@/lib/bucket/schema";
import { suggestedPlanType } from "@/lib/bucket/grouping";
...
/**
 * Creates or edits a plan. Given a `source` bucket idea, it opens already
 * filled from that idea - title, story, place, target date, estimated cost, and
 * a kind guessed from its category - and the saved plan stays linked to it.
 */
export function PlanForm({ defaultTimezone, plan, source }: { defaultTimezone: string; plan?: Plan; source?: BucketItem }) {
  ...
  return (
    <form action={action} onChange={event => previewTime(event.currentTarget)} onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); startTransition(() => action(data)); }} className="space-y-6" noValidate>
      ...
      {source ? <input type="hidden" name="sourceBucketId" value={source.id}/> : null}
      <div className="space-y-2"><Label htmlFor="title">What are you planning?</Label><Input id="title" name="title" defaultValue={plan?.title ?? source?.title} placeholder="A quiet dinner, a day trip…" required {...errorProps("title")} />{fieldError("title")}</div>
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="space-y-2"><Label htmlFor="type">Kind of plan</Label><select id="type" name="type" {...errorProps("type")} className={fieldClass} defaultValue={plan?.type ?? (source ? suggestedPlanType(source.category) : "date")}>{planTypes.map((type) => <option key={type} value={type}>{planTypeLabel(type)}</option>)}</select>{fieldError("type")}</div>
        <div className="space-y-2"><LocationField defaultValue={plan?.location ?? source?.location ?? ""} invalid={!!state.fields?.location}/>{fieldError("location")}</div>
      </div>
      {/* A plan made from an idea is saved through create_plan_from_bucket, which
          stores no map link; offering the field there would drop it silently. */}
      {source ? null : <details><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold text-primary">Map link (optional)</summary>
