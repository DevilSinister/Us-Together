...
export function GalleryWorkspace({initial,scope}:{initial:GalleryPage;scope?:{kind:"memory"|"moment";entry:string}}){
 ...
 for(const u of urls.current)URL.revokeObjectURL(u);urls.current=loaded.flatMap(m=>[m.url,m.previewUrl].filter((u):u is string=>!!u));setItems(current=>same(current,loaded)?current:loaded);setNext(cursor);setError("");
 ...
 <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 xl:grid-cols-6">{g.items.map(m=>{const position=visible.indexOf(m);return <MediaTile key={m.sortKey} item={m} eager={position<6} label={"Open "+(m.media_type==="image"?"photo":"video")+" "+(position+1)} onOpen={()=>setSelected(m.sortKey)}/>;})}</div></section>)}
