...
export function MediaViewer({items,index,onIndex,onClose,onChange}:{items:GalleryItem[];index:number;onIndex:(i:number)=>void;onClose:()=>void;onChange:()=>Promise<void>}){
 ...
 {item?<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]"><div className="min-w-0">{item.media_type==="image"?<div aria-label="Swipe photo" className="relative h-[42dvh] rounded-xl bg-secondary lg:h-[65dvh]" style={{touchAction:"pan-y pinch-zoom"}} onTouchStart={e=>{touch.current=e.touches.length===1?{x:e.touches[0].clientX,y:e.touches[0].clientY}:null;}} onTouchCancel={()=>{touch.current=null;}} onTouchEnd={e=>{const start=touch.current;touch.current=null;if(!start||e.changedTouches.length!==1)return;const dx=e.changedTouches[0].clientX-start.x,dy=e.changedTouches[0].clientY-start.y;if(Math.abs(dx)>=50&&Math.abs(dx)>Math.abs(dy)*1.5)onIndex((index+(dx<0?1:items.length-1))%items.length);}}><Image src={mediaSource(item)} fill unoptimized sizes="(max-width:1024px) 95vw, 65vw" className="object-contain" alt={item.caption||"Selected photo"} referrerPolicy="no-referrer"/></div>:<video key={item.id} controls playsInline preload="metadata" className="max-h-[65dvh] w-full rounded-xl bg-secondary" src={mediaSource(item)} aria-label={item.caption||"Selected video"}><track kind="captions"/></video>}
