...
function FileOptions({item,onChange,onRemove}:{item:GalleryItem;onChange:()=>Promise<void>;onRemove:()=>void}){
 ...
 return <details className="mt-4"><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold text-primary">Caption and file options</summary>{error?<p role="alert" className="status-message status-error">{error}</p>:null}<form method="post" onSubmit={e=>{e.preventDefault();void mutate("caption",String(new FormData(e.currentTarget).get("caption")??""));}} className="space-y-3"><Label htmlFor={"caption-"+item.id}>Caption</Label><Input id={"caption-"+item.id} name="caption" defaultValue={item.caption} maxLength={240}/><Button variant="outline" disabled={pending}>Save caption</Button></form><div className="mt-4"><ConfirmDelete variant="ghost" label="Remove file" title="Remove this file?" description="The photo or video and every comment on it are deleted for both of you." onConfirm={async()=>{if(item.access.previewSession)await changePreviewMedia(item.access,item.id,undefined);else{const r=await memoryMediaAction({operation:"remove",kind:item.access.kind,memoryId:item.access.id,id:item.id});if(r.error)return r.error;}onRemove();await onChange();}}/></div></details>;
}
