...
export function MilestoneForm({previewSession}:{previewSession?:string}) {
  ...
  return <form action={action} className="space-y-6" noValidate>
    ...
    <label className="flex min-h-12 items-center gap-3 rounded-panel bg-secondary px-5 py-4 text-sm font-semibold"><input type="checkbox" name="featured" className="size-5 accent-[var(--primary)]" /><Star className="size-4 text-primary" aria-hidden="true" />Feature this moment on Home</label>
    ...
    <SubmitButton>Save the moment</SubmitButton>
  </form>;
}
