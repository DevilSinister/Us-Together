...
function scope(a:EntryAccess){if(!a.previewSession)throw Error("Preview session required.");return a.previewSession+":"+a.kind+":"+a.id;}
function open():Promise<IDBDatabase>{return new Promise((resolve,reject)=>{const r=indexedDB.open("us-together-preview-media",1);r.onupgradeneeded=()=>{for(const name of ["assets","comments"]){const s=r.result.createObjectStore(name,{keyPath:"id"});s.createIndex("scope","scope");}};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(Error("Browser storage is unavailable. Allow site storage and try again."));});}
async function rows<T>(store:string,key:string):Promise<T[]>{const db=await open();try{return await new Promise((resolve,reject)=>{const r=db.transaction(store).objectStore(store).index("scope").getAll(key);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(Error("Could not read preview files."));});}finally{db.close();}}
...
export async function addPreviewComment(a:EntryAccess,body:string,mediaId?:string){if(mediaId&&!(await rows<Asset>("assets",scope(a))).some(x=>x.id===mediaId&&x.expires>Date.now()))throw Error("Photo unavailable.");const content=body.trim();if(!content||content.length>2000)throw Error("Write a comment under 2,001 characters.");if((await previewComments(a,mediaId)).length>=500)throw Error("Comment limit reached.");const id=crypto.randomUUID();await write("comments",id,{id,scope:scope(a),mediaId,body:content,created_at:new Date().toISOString(),mine:true,expires:Date.now()+86400000});}
export async function deletePreviewComment(a:EntryAccess,id:string,mediaId?:string){if(!(await previewComments(a,mediaId)).some(x=>x.id===id))throw Error("Comment unavailable.");await write("comments",id);}
