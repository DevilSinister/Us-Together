-- Hosted verification gate. Fixtures and temporary pgTAP installation always roll back.
DO $verify$
...
$suite$;
    SELECT string_agg(result,E'\n') INTO failures FROM bucket_tap_results WHERE result LIKE 'not ok%' OR result LIKE '# Looks like%';
    IF failures IS NOT NULL THEN RAISE EXCEPTION 'Phase 4 verification failed: %',failures; END IF;
    RAISE EXCEPTION USING ERRCODE='P4444', MESSAGE='Phase 4 assertions passed; roll back fixtures.';
  EXCEPTION WHEN SQLSTATE 'P4444' THEN NULL;
  END;
END
$verify$;
