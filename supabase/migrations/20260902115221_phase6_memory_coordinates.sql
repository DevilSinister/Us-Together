-- Complete optional memory coordinates using existing paired/range constraints.
create or replace function public.update_memory_details(input jsonb) returns jsonb language plpgsql security invoker set search_path='' as $$
...
begin
 ...
 update public.memories set title=btrim(input->>'title'),description=nullif(input->>'description',''),memory_date=(input->>'memoryDate')::date,location=nullif(input->>'location',''),latitude=case when input ? 'latitude' then (input->>'latitude')::numeric else m.latitude end,longitude=case when input ? 'longitude' then (input->>'longitude')::numeric else m.longitude end,rating=(input->>'rating')::smallint,is_favorite=(input->>'favorite')::boolean where id=m.id;
...
-- Coordinates are paired, bounded and updated through the same revision-checked RPC.
perform set_config('request.jwt.claim.sub','60000000-0000-4000-8000-000000000001',true);
insert into memory_tap_results select is((public.update_memory_details(jsonb_build_object('id','62000000-0000-4000-8000-000000000001','version',(select version from public.memories where id='62000000-0000-4000-8000-000000000001'),'title','Coordinate fixture','memoryDate','2026-08-20','tags','[]'::jsonb,'favorite',false,'latitude',24.86,'longitude',67.01))->>'ok'),'true','coordinates update through authorized RPC');
insert into memory_tap_results select is((select latitude from public.memories where id='62000000-0000-4000-8000-000000000001'),24.86::numeric,'latitude persisted');
insert into memory_tap_results select is((select longitude from public.memories where id='62000000-0000-4000-8000-000000000001'),67.01::numeric,'longitude persisted');
insert into memory_tap_results select throws_ok($$update public.memories set latitude=null where id='62000000-0000-4000-8000-000000000001'$$,'23514',null,'incomplete coordinates rejected');
insert into memory_tap_results select throws_ok($$select public.update_memory_details(jsonb_build_object('id','62000000-0000-4000-8000-000000000001','version',(select version from public.memories where id='62000000-0000-4000-8000-000000000001'),'title','Invalid coordinates','memoryDate','2026-08-20','tags','[]'::jsonb,'favorite',false,'latitude',91,'longitude',67.01))$$,'23514',null,'out-of-range coordinates rejected by RPC');
...
if assertion_count <> 42 then raise exception 'Security assertions did not all execute';end if;
