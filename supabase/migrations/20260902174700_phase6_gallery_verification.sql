do $verify$ declare failures text; n integer; begin begin
...
select string_agg(result,E'\n'),count(*) into failures,n from entry_tap_results where result like 'not ok%';
if failures is not null then raise exception '%',failures;end if;
raise exception using errcode='P6009',message='Assertions passed; rollback fixtures';
exception when sqlstate 'P6009' then null;end;
if exists(select 1 from auth.users where id::text like '70000000-%') or to_regclass('pg_temp.entry_tap_results') is not null then raise exception 'Fixture rollback failed';end if;
end $verify$;
