-- Exception rollback semantics: https://www.postgresql.org/docs/current/plpgsql-control-structures.html#PLPGSQL-ERROR-TRAPPING
-- Rollback-only security verification: failures abort; fixtures never persist.
do $verify$
...
if assertion_count <> 32 then raise exception 'Security assertions did not all execute';end if;
