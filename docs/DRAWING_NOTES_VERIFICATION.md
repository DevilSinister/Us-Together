...
### Signed release APK — 2026-09-21

Built by the owner in a terminal with `set-build-env.local.ps1` (gitignored) supplying the public values and prompting for the keystore password. First attempt failed at `packageRelease` with a missing `storePassword`: the script used `ConvertFrom-SecureString -AsPlainText`, a PowerShell 7 flag absent from Windows PowerShell 5.1, so the variable was empty; the committed example is fixed to unwrap the SecureString via BSTR. Second attempt: `BUILD SUCCESSFUL`, `app/build/outputs/apk/release/app-release.apk` (≈5.1 MB). `apksigner verify --print-certs`: signer SHA-256 `9b8b00ed1913db9c14cdd33184409aa8fb2fdf5c913fdd3ba7306cfcfcca9618`, which matches the fingerprint in `public/.well-known/assetlinks.json` on the deployed origin. `aapt dump badging`: package `app.ustogether`, versionCode 2, versionName 0.2.0, launchable activity `com.google.androidbrowserhelper.trusted.LauncherActivity`. Installation and the device script remain owner steps.
