...
The hosted project already has `20260917013807_drawing_notes.sql` and `20260917014059_drawing_notes_author_index.sql` applied. Confirm both entries in the migration ledger before deploying dependent web code. The private Android project is in `android-widget/`; see its README for public build variables, the release keystore, and the Trusted Web Activity origin. Android push is dispatched from the database through the `fcm-dispatch` Edge Function (migration `20260920100500_fcm_deliveries.sql`); `FIREBASE_SERVICE_ACCOUNT_JSON` and `PUSH_DISPATCH_SECRET` are Edge Function secrets, never web-server or `NEXT_PUBLIC_*` values, and the Vault secret `fcm_endpoint_url` turns the worker on. Without them, drawings can still be sent and viewed and the Android widget refreshes on app open, reconnection and its periodic job. Real-account RLS and Storage tests are required before release.
