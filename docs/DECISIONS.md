...
## ADR-037 — Pushes update installed apps: the web by prompt, the APK from GitHub Releases

**Status:** Implemented, 2026-09-25.

**Decision:** Two update paths, because the app is two things. The web app — which is also everything the Android Trusted Web Activity shows — is redeployed by Vercel on each push; open tabs poll `/api/version` on focus, visibility, reconnection and every five minutes, show "A new version is ready" with *Refresh* and *Later*, and reload on their own at the next in-app page change. The sideloaded APK is rebuilt, signed and published to a GitHub release by `.github/workflows/android-release.yml` on each push to `main` that touches `android-widget/`; the app checks `releases/latest/download/update.json` on launch (throttled to 30 minutes) and every six hours, asks before opening the TWA on a plain launch, notifies once per release, and downloads, verifies and installs through `PackageInstaller`.

**Consequences:** No automatic reload happens mid-page, because a reload discards a half-written note or drawing; a page change is the one moment nothing on screen is lost. The version string is the commit SHA rather than a timestamp, because `next.config.ts` can be evaluated more than once per build and a timestamp could differ between the client and server bundles and prompt forever. GitHub Releases is used because the repository is public and needs no token or new infrastructure; the cost is that each release APK — carrying the web origin and the public Supabase and Firebase client values, all of which the deployed web app already exposes — is publicly downloadable. The very first self-update always shows Android's own confirmation sheet, since the installer of record is whatever sideloaded the APK; after it, on Android 12+, `USER_ACTION_NOT_REQUIRED` lets later updates install without one. CI must sign with the same keystore as the APK already on the phones, so the release workflow refuses to run without the signing secrets instead of publishing an update that cannot install. Debug builds never check. The launcher subclasses `LauncherActivity` and returns `false` from `shouldLaunchImmediately()` only when an update is waiting on a plain `MAIN` launch, so notification deep links never meet a dialog.


## ADR-038 — Deletion is a confirmation dialog at the foot of the page

**Status:** Owner-requested, implemented 2026-09-25.

**Decision:** Every detail page ends with one action row — Edit, then Delete — after the thing itself (`EntryActions`). Delete is always `ConfirmDelete`: a danger-outline button that opens a modal naming what goes and what stays, with focus on "Keep it" so Enter never deletes. The typed-DELETE disclosures on memories, plans, notes, wishes, bucket ideas and lists are gone. Comments, gallery files, plan attachments and private gift plans, which used to delete on a single tap, now confirm through the same dialog. Preconditions the server enforces (a memory's files, a plan's attachments, a list's ideas) are named inside the dialog and disable its confirm button rather than failing after the click.

**Consequences:** The server contracts are unchanged: `deleteMemory`, `mutatePlan` and `mutateBucket` still require the literal `DELETE`, and the dialog sends it only after the partner confirms, so a stray call still cannot delete. Checklist steps, bucket steps and reminders stay one-tap removals: they are small, easily re-added, and a dialog per step would make a checklist tedious. Couple and account deletion keep their typed confirmation, because account-scale deletion still calls for strong confirmation (UX_FLOWS). A confirmation opened inside another dialog (list deletion inside Options, file removal inside the viewer) stacks natively; `DialogContent` now ignores Escape and Tab events that bubble up the React tree from a portalled child, so only the top dialog responds.

## ADR-039 — An opened bucket list is sectioned, and a plan can start from any idea

**Status:** Owner-requested, implemented 2026-09-25.

**Decision:** An opened list groups its ideas by status by default — In progress, Planned, Ideas, Completed, with completed last so the list reads forward — and an "Organize" control beside Options regroups by category or priority. The choice is a per-device preference in `localStorage`, read through `useSyncExternalStore` with an in-memory fallback. Pagination appends ("Load more ideas") instead of paging, so sections regroup as ideas arrive, and partner refresh reloads the whole window already on screen. Ideas render as tiles carrying a category mark, a four-dot priority meter, and target date and place, or, once lived, the day it was lived. The new-plan page offers every not-yet-completed idea in a picker. Choosing one reloads `/plans/new?bucket=` with the full plan form prefilled from the idea, and the kind is guessed from its category.

**Consequences:** `BucketConversionForm` now only makes memories; plans made from ideas use `PlanForm` with a `source`, so both routes into a plan share one form and its validation. `create_plan_from_bucket` stores no map link, so the map-link field is hidden when a plan has a source rather than silently dropped. Grouping is client-side over the loaded pages, which is correct because a list page already holds every idea that has been loaded.

## ADR-040 — The partner appears as you named them, everywhere, and profile pictures are framed before upload

**Status:** Owner-requested, implemented 2026-09-25.

**Decision:** Every label that names the partner resolves through one rule: the name you chose for them in Settings, then their own profile name, then "Your partner". `partnerName()` in `src/lib/couple/context.ts` — used by notes, drawings and wishes — now reads `partner_presentations` first, where it used to read the partner's profile directly; Home's note and drawing lines and the pairing card use the resolved identities. The rule is symmetric: each partner sees the other as they themselves chose, and nobody sees the name chosen for them. Avatar addresses carry a `?v=` version, a short FNV-1a hash of the stored object path, so a newly saved picture replaces the old one immediately instead of after the route's five-minute browser cache. Every profile-picture input — your own at onboarding and in Settings, and your partner's — opens an editor (`AvatarField`): drag to pan, pinch, wheel or slider to zoom (1–4×), rotate by quarter turns, Center and Reset, with the circle showing the exact cut. The browser draws the framing onto a 640px canvas and submits that JPEG under the existing field name.

**Consequences:** No schema, policy or bucket change: the server actions still validate type and size, and the bucket's own limits still apply; a 640px JPEG is a small fraction of the 2 MB cap, so the chooser accepts any image the browser can decode. The original photo never leaves the device. Settings can now change your own photo, so `updateProfile` accepts an optional `avatar` file. Both picture-saving actions remove the replaced object from the caller's own folder after the row points at the new one; a failed removal leaves a harmless orphan. The version hash never exposes the storage path, which carries a user id. The crop geometry is pure and unit-tested (`src/lib/avatar/crop.ts`).

## ADR-041 — Photos open full screen and swipe; file actions live behind a menu

**Status:** Owner-requested, implemented 2026-09-25. Supersedes the paper-dialog viewer described in the gallery surface brief.

**Decision:** The media viewer is a full-screen black stage. The previous, current and next files are mounted side by side, so a sideways swipe drags the next photo in under the finger. Nothing wraps: the first and last files push back. Pinch and double-tap zoom, a swipe up opens the comments, and a long pull down closes. The top bar — close, the source memory or moment with "n of m" and its date, and a three-dot menu with Edit (or Add) caption, Download, Open memory and Delete — is hidden until a tap, a mouse movement or a Tab. When hidden it takes no pointer events. The bottom carries only the caption and a Comments handle, and comments and the caption editor rise in a bottom sheet. When a swipe crosses into a different memory or moment, its title rises briefly at the top and is announced politely. The gallery hands the viewer its files in the order the grid reads, group by group. Pointer users get previous and next arrows. Arrow keys navigate and Escape closes the menu, then the sheet, then the viewer.

**Consequences:** Delete keeps ADR-038's confirmation. `ConfirmDelete` gained a controlled, trigger-less mode for menu use, and after a delete the viewer lands on a neighbouring file. Settling after a swipe is timed, not driven by `transitionend`, because a backgrounded tab never finishes a transition. Reduced motion navigates instantly. Pure gesture decisions are unit-tested in `src/lib/entries/viewer.ts`. Taps on a video still reach its own controls: pointer capture waits until a swipe actually starts, and the lower 64px of a video is left to its scrubber.

## ADR-042 — Our Story is told as chapters of prints

**Status:** Owner-requested, implemented 2026-09-25.

**Decision:** Our Story opens on both faces and names (as chosen in Settings), with the day it began and what the couple has kept as a single sentence rather than metric tiles. Each calendar year is a chapter, numbered from the year the story began, with month markers inside it. Memories and moments lead with up to three of their own photos as a small tilted stack of prints, followed by the opening of their story and who kept them. Plans kept and dreams lived are quieter lines on the same thread. The last page ends on "Where it began" and the start date.
